...
void FeatureUtil::calculateIntegralImage(FeatureUtil::FeatureData& featureData) {

        // If there's no image data, the function doesn't output anything
    if (featureData.originalData.empty()) return;

    // Ensure width and height make sense given the size of originalData
    if (height * width != featureData.originalData.size()) {
        std::cerr << "Error: Dimensions do not match the size of originalData\n";
        return;
    }
    ...
    integralData[0][0] = featureData.originalData[0];
    ...
    for (int x = 1; x < width; ++x) {
        ...
        integralData[0][x] = integralData[0][x-1] + featureData.originalData[x];
    }
    ...
    for (int y = 1; y < height; ++y) {
                ...
                integralData[y][0] = integralData[y-1][0] + featureData.originalData[y * width];
    }
    ...
    for (int y = 1; y < height; ++y) {
        for (int x = 1; x < width; ++x) {
            ...
            integralData[y][x] = featureData.originalData[y * width + x] +
                                 integralData[y-1][x] +
                                 integralData[y][x-1] -
    ...
    featureData.integralData = integralData;
}
...
// Generates all the data used for our sample images, so it can be used to train the algorithm and test on our sample set
std::vector<FeatureUtil::FeatureData> FeatureUtil::generateFeatures(std::vector<std::vector<uint8_t>> examples, std::vector<int> labels){


  // Initialize the base variables used to store our different Haar-Features based upon our sample images

  // Generates the dimensions of all 160,000 of the Haar-Features

  // We go through each of the sample images ("examples"), running through an exhaustive search of 160,000 features

  int baseRes = width;
  std::vector<tuple<FeatureType, int, int, int, int>> featureConfigs = FeatureUtil::generateFeatureConfigs(baseRes);
  std::vector<FeatureData> imageFeatures;

  for(int i = 0; i < examples.size(); i ++) {
    printf("processing example %d of %ld\n", i+1, examples.size());
    FeatureData featureData;

        // Inputs our sample image into our featureData
    featureData.originalData = examples[i];

        // Determines whether the image has a face(1) or not (0)
    featureData.label = labels[i];

        // Calculates the integral image of our specific sample image
    calculateIntegralImage(featureData);

        // Calculates the "contrast" value of all 5 different Haar-Features in our original image data
    featureData.featureEvaluations = generateFeatureEvaluations(featureData.integralData, featureConfigs);

        // Places it back into the vector for use
    imageFeatures.emplace_back(featureData);
  }


  writeFeatureData("feature_outputs.bin", imageFeatures);

  // Return the imageFeatures vector in order for the algorithm to train properly using Adaboost
